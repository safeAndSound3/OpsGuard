...
function Sidebar() { const items = [['overview', '监控总览', '/'], ['inspection', '巡检任务', '/inspection'], ['alert', '平台告警', '/alerts'], ['data', '数据源', '/datasources'], ['settings', '系统配置', '/config']]; return <aside className="sidebar"><div className="brand"><img className="brand-logo" src="/favicon.svg" alt="" /><div><b>OpsGuard</b><small>巡检平台</small></div></div><nav>{items.map(([icon, label, path]) => <NavLink key={path} end={path === '/'} to={path} className={({ isActive }) => `nav-link ${isActive ? 'active' : ''}`}><Icon name={icon} /><span>{label}</span></NavLink>)}</nav><div className="sidebar-footer"><span className="online-dot" /><span>23 / 26 节点在线</span><small>采集服务运行正常</small></div></aside> }
function Topbar() { return <header className="topbar"><div><h1>监控与采集管理</h1><p className="top-desc">统一管理巡检任务、告警规则与数据源连接状态。</p></div><div className="top-actions"><button className="icon-button" aria-label="通知"><Icon name="bell" /><i>7</i></button><div className="avatar">OP</div><div className="account"><b>平台运维部</b><span>生产环境</span></div></div></header> }
function Dashboard() { const [metrics, setMetrics] = useState(fallbackMetrics); const [loading, setLoading] = useState(false); const refresh = async () => { setLoading(true); try { const r = await fetch(`${api}/overview`); const d = await r.json(); if (Array.isArray(d.metrics)) setMetrics(d.metrics.map((m: any, i: number) => ({ ...fallbackMetrics[i % 4], label: m.label ?? m.name ?? fallbackMetrics[i % 4].label, value: m.value ?? fallbackMetrics[i % 4].value }))) } catch { /* mock snapshot stays visible */ } finally { setLoading(false) } }; useEffect(() => { void refresh() }, []); return <div className="page dashboard"><section className="hero"><div><h2>平台运行平稳，<em>服务健康。</em></h2><p>系统已连续稳定运行 32 天，关键业务链路处于预期区间。</p></div><button className="button secondary" onClick={refresh} disabled={loading}>{loading ? '同步中…' : '刷新数据'} <Icon name="arrow" /></button></section><section className="metric-grid">{metrics.map((m) => <article className={`metric-card ${m.tone}`} key={m.label}><div className="metric-top"><span>{m.label}</span><span className="metric-symbol">⌁</span></div><strong>{m.value}</strong><div className="metric-foot"><span>{m.detail}</span><b>{m.change}</b></div></article>)}</section><section className="content-grid"><article className="surface chart-card"><SectionTitle title="平台吞吐量" action="近 12 小时" /><div className="chart"><div className="chart-labels"><span>120k</span><span>80k</span><span>40k</span><span>0</span></div><div className="chart-bars">{[34, 48, 42, 61, 58, 82, 70, 91, 65, 73, 86, 96].map((n, i) => <div className="bar-col" key={i}><i style={{ height: `${n}%` }} /></div>)}</div></div><div className="chart-axis"><span>00:00</span><span>04:00</span><span>08:00</span><span>12:00</span></div></article><article className="surface health-card"><SectionTitle title="服务健康度" /><div className="health-ring"><span>98<small>健康评分</small></span></div><div className="health-stats"><p><span>●</span>在线服务 <b>23 / 26</b></p><p><span>●</span>采集任务 <b>18 / 21</b></p><p><span>●</span>数据源健康 <b>12 / 13</b></p></div></article></section><section className="surface activity"><SectionTitle title="近期告警" action="查看全部" /><div className="activity-list"><Activity tone="danger" title="支付服务响应时间超过阈值" meta="支付链路 · 4 分钟前" /><Activity tone="warning" title="缓存集群容量已使用 82%" meta="Redis 集群 · 18 分钟前" /><Activity tone="info" title="订单库巡检任务已完成" meta="自动巡检 · 32 分钟前" /></div></section></div> }
...
function DataSources() {
  ...
  return <div className="page"><PageHead title="数据源" description="管理平台采集的数据连接，并及时处理异常源。" action="添加数据源" onAction={openCreateModal} />{sources.length === 0 ? <section className="surface empty-state"><b>暂无数据源</b><span>点击右上角添加数据源，完成连接测试后即可保存。</span></section> : <section className="source-grid">{sources.map(s => <article className="surface source-card" key={s.id}><div className="source-card-top"><span className="source-logo">{s.type.slice(0, 1)}</span><span className={`tag ${s.status === '健康' ? 'success' : 'pending'}`}>{s.status}</span></div><h3>{s.name}</h3><p>{s.type} · {s.host}:{s.port}</p>{s.remark && <small className="source-remark">{s.remark}</small>}<footer><span>上次检测：{s.lastTest}</span><div className="source-actions"><button type="button" onClick={() => openEditModal(s)}>编辑</button><button className="danger" type="button" onClick={() => void deleteSource(s)}>删除</button></div></footer></article>)}</section>}{modalOpen && <div className="modal-backdrop" role="presentation" onMouseDown={closeModal}><section className="surface source-modal" role="dialog" aria-modal="true" aria-labelledby="source-modal-title" onMouseDown={(event) => event.stopPropagation()}><header className="modal-head"><div><h2 id="source-modal-title">{editingSource ? '编辑数据源' : '添加数据源'}</h2></div><button className="close-button" type="button" aria-label="关闭" onClick={closeModal}>×</button></header><form key={`${editingSource?.id || 'new'}-${sourceType}`} onSubmit={saveSource}><div className="type-picker" role="group" aria-label="数据类型">{sourceTypes.map(type => <button key={type} type="button" className={sourceType === type ? 'active' : ''} onClick={() => { setSourceType(type); if (!editingSource || editingSource.type !== type) setOptionRows([{ key: '', value: '' }, { key: '', value: '' }]) }}>{type}</button>)}</div><div className="modal-form"><Field label="数据源名称" name="name" value={editingSource?.name || `${sourceType} 生产数据源`} required /><label>主机地址 <span className="required-mark">*</span><input name="host" defaultValue={editingSource?.host || ''} placeholder="例如 127.0.0.1 或 broker.internal" required /></label><label>端口 <span className="required-mark">*</span><input name="port" defaultValue={editingSource?.port || defaultPorts[sourceType]} required /></label>{sourceType === 'Kafka' ? <label>Topic / Consumer Group<input name="topic" defaultValue={editingSource?.database || ''} placeholder="例如 ops-events / ops-monitor" /></label> : <label>数据库 / 命名空间<input name="database" defaultValue={editingSource?.database || ''} placeholder={sourceType === 'Redis' ? '例如 0' : '例如 opsguard_lab'} /></label>}<label>用户名<input name="username" defaultValue={editingSource?.username || ''} placeholder={sourceType === 'Redis' ? '可选' : '请输入用户名'} /></label><label>密码{!editingSource && <span className="required-mark"> *</span>}<input name="password" type="password" required={!editingSource} placeholder={editingSource ? '留空则不修改密码' : '请输入密码'} /></label>{sourceType === 'Elasticsearch' && <label>索引前缀<input name="indexPrefix" placeholder="例如 logs-*" /></label>}<label className="wide">备注<textarea name="remark" defaultValue={editingSource?.remark || ''} placeholder="记录用途、负责人、环境或注意事项" /></label><div className="wide option-editor"><div><b>连接参数</b><span>示例：ssl true、timeout 10s、brokers host1:9092,host2:9092</span></div>{optionRows.map((row, index) => <div className="option-row" key={index}><input aria-label="参数名" placeholder="key" value={row.key} onChange={(event) => setOptionRows(rows => rows.map((item, i) => i === index ? { ...item, key: event.target.value } : item))} /><input aria-label="参数值" placeholder="value" value={row.value} onChange={(event) => setOptionRows(rows => rows.map((item, i) => i === index ? { ...item, value: event.target.value } : item))} /></div>)}<button className="text-button" type="button" onClick={() => setOptionRows(rows => [...rows, { key: '', value: '' }])}>添加参数 <Icon name="plus" /></button></div></div><footer className="modal-actions"><button className="button secondary" type="button" onClick={(event) => { const form = event.currentTarget.form; if (form) void testConnection(form) }} disabled={testing}>{testing ? '测试中...' : '测试连接'}</button><button className="button" type="submit" disabled={saving}>{saving ? '保存中...' : '保存'}</button></footer></form></section></div>}{message && <div className="toast">{message}</div>}</div>
}
