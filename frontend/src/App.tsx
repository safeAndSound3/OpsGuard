...
function Alerts() {
  ...
  const loadSchema = async (sourceId: string, preferredDatabase = '', preferredTable = '', preferredField = '') => {
    ...
    try {
      ...
      const filteredSchema = monitoredDatabases.length > 0
        ? Object.fromEntries(Object.entries(nextSchema).filter(([database]) => monitoredDatabases.includes(database)))
        : nextSchema
      const databases = Object.keys(filteredSchema)
      const database = preferredDatabase && filteredSchema[preferredDatabase] ? preferredDatabase : databases[0] || ''
      const tables = database ? Object.keys(filteredSchema[database] || {}) : []
      const table = preferredTable && tables.includes(preferredTable) ? preferredTable : tables[0] || ''
      const fields = database && table ? filteredSchema[database]?.[table] || [] : []
  ...
  return (
    <div className="page">
      ...
      {modalOpen && (
        <div className="modal-backdrop" role="presentation" onClick={closeRuleModal}>
      ...
      {deleteRuleTarget && (
        <div className="modal-backdrop" role="presentation" onClick={() => setDeleteRuleTarget(null)}>
