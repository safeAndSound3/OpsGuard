...
function Dashboard() {
  ...
  return <div className="page dashboard"><section className="hero"><div><h2>{selectedDashboardId ? 'MySQL 监控大屏' : '监控总览'}</h2><p>{selectedDashboardId ? '当前展示单个 MySQL 实例大屏，数据每 15 秒同步。' : '当前只展示已导入的大屏名称和关键运行数据。点击左侧二级菜单查看单实例大屏。'}</p></div><button className="button secondary" onClick={refresh} disabled={loading}>{loading ? '同步中...' : '刷新数据'} <Icon name="arrow" /></button></section>{selectedDashboardId ? (selectedDashboard ? <section className="mysql-dashboard-stack"><MySQLDashboard data={selectedDashboard} onDelete={() => setDeleteTarget(selectedDashboard)} /></section> : <section className="surface dashboard-empty"><b>未找到该大屏</b><span>该大屏可能尚未导入、对应节点尚未采集成功，或已经被删除。</span></section>) : <MonitorOverview instances={mysqlInstances} />}{deleteTarget && <div className="modal-backdrop" role="presentation" onMouseDown={() => setDeleteTarget(null)}><section className="surface confirm-modal" role="dialog" aria-modal="true" aria-labelledby="delete-dashboard-title" onMouseDown={(event) => event.stopPropagation()}><header className="modal-head"><div><h2 id="delete-dashboard-title">确认删除大屏</h2></div><button className="close-button" type="button" aria-label="关闭" onClick={() => setDeleteTarget(null)}>×</button></header><p>确认删除“{deleteTarget.displayName}”吗？删除后不会影响 MySQL 数据节点和采集数据。</p><footer className="modal-actions"><button className="button secondary" type="button" onClick={() => setDeleteTarget(null)}>取消</button><button className="button danger-button" type="button" onClick={() => deleteDashboard(deleteTarget.status.sourceId)}>确认</button></footer></section></div>}</div>
}
function MonitorOverview({ instances }: { instances: MySQLInstanceStatus[] }) {
  const imported = getImportedDashboards()
  const dashboards = imported.map(config => ({ config, status: instances.find(item => item.sourceId === config.sourceId) }))
  const healthy = dashboards.filter(item => item.status?.status === '健康').length
  const latest = dashboards.map(item => new Date(item.status?.lastCollectedAt || '').getTime()).filter(Number.isFinite).sort((a, b) => b - a)[0]
  return <section className="overview-stack">{dashboards.length === 0 ? <div className="surface dashboard-empty"><b>暂无已导入大屏</b><span>在数据节点中点击“导入大屏”后，这里会展示大屏名称和关键运行数据。</span></div> : <><div className="overview-kpis"><DashboardKpi label="已导入大屏" value={String(dashboards.length)} detail="当前监控总览范围" /><DashboardKpi label="健康大屏" value={String(healthy)} detail="关联实例状态健康" /><DashboardKpi label="异常大屏" value={String(dashboards.length - healthy)} detail="需要关注或等待采集" /><DashboardKpi label="最近采集" value={latest ? formatCollectedAt(new Date(latest).toISOString()) : '待采集'} detail="已导入大屏最新采集时间" /></div><div className="overview-node-list">{dashboards.map(({ config, status }) => <article className="surface overview-node overview-dashboard" key={config.sourceId}><div><b>{config.name}</b><span>{status ? `存活 ${formatDuration(status.uptimeSeconds)} · 连接 ${percent(status.threadsConnected, status.maxConnections)}% · 慢查询 ${status.slowQueries}` : '等待采集数据'}</span></div><span className={`tag ${status?.status === '健康' ? 'success' : 'pending'}`}>{status?.status || '待采集'}</span><time>{formatCollectedAt(status?.lastCollectedAt || '')}</time></article>)}</div></>}</section>
}
